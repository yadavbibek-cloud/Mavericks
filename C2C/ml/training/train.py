...
from C:\\GridSense\\C2C\\dataset\\ (node_samples.csv and edges.csv).
...
def train_gridsense_model(
    dataset_dir: str = r"C:\GridSense\C2C\dataset",
    output_model_path: str = "models/gridsense_gnn.pt",
    output_metrics_path: str = "results/metrics.json",
    epochs: int = 10,
    batch_size: int = 64,
    lr: float = 0.003,
    max_train_scenarios: int = 400
):
    ...
    scenario_edges = {}
    for scen_id, grp in df_edges.groupby('scenario_id'):
        scenario_edges[scen_id] = grp
    ...
    train_scenarios = train_df['scenario_id'].unique()[:max_train_scenarios]
    val_scenarios = val_df['scenario_id'].unique()[:100]
    test_scenarios = test_df['scenario_id'].unique()[:100]
    ...
    with open("models/feature_norm.json", "w") as f:
    ...
    def build_graph(scen_id, df_subset):
        scen_nodes = df_subset[df_subset['scenario_id'] == scen_id]
        if len(scen_nodes) == 0:
            return None
    ...
    for epoch in range(1, epochs + 1):
        ...
        for scen_id in train_scenarios:
            graph = build_graph(scen_id, train_df)
            if graph is None:
                continue
        ...
        with torch.no_grad():
            for scen_id in val_scenarios:
                graph = build_graph(scen_id, val_df)
                if graph is None:
                    continue
                x, edge_index, edge_attr, y_fail, y_ttc, y_peak = graph
                out = model(x, edge_index, edge_attr)
    ...
    with torch.no_grad():
        for scen_id in test_scenarios:
            graph = build_graph(scen_id, test_df)
            if graph is None:
                continue
            x, edge_index, edge_attr, y_fail, y_ttc, y_peak = graph
            out = model(x, edge_index, edge_attr)
